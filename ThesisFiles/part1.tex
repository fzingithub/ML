...
\textbf{信息增益}~~特征~A~对训练数据集~D~的信息增益$~g(D,A）~$ ， 定义为集合D的经验熵 $~H(D）~$与特征~A~给定条件下~D~的经验条件熵 $~H(D|A)~$ 之差，即$$g(D|A) = H(D)-H(D|A)$$
\indent一般地， 熵$~H(Y)~$与条件熵$~H(Y|X)~$之差称为互信息（mutual information）.决策树学习中的信息增益等价于训练数据集中类与特征的互信息，
...
\textcolor{red}{ $C(T)$表示模型对训练数据的预测误差，即模型与训练数据的拟合程度，$|T|~$表示模型复杂度，参数$~\alpha\geq 0~$控制两者之间的影响}．较大的$~\alpha~$促使选择较简单的模型（树〉，较小的$~\alpha~ $促使选择较复杂的模型（树〉。$\alpha=0$意味着只考虑模型与训练数据的拟合程度，不考虑模型的复杂度
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.8\textwidth]{tree.eps}
%  \caption{}\label{}
\end{figure}
...
\begin{figure}[!h]
  \centering
  \includegraphics[width=0.85\textwidth]{decisionTree.eps}
%  \caption{}\label{}
\end{figure}
...
\begin{align}
   ...
   \text{求解：} &\textcircled{\footnotesize{1}} ~\text{先最小化lagrangian.$~L(w,b,\alpha)~$关于w和b.}  \nonumber\\
                ...
                \theta_D(\alpha)&=~\min \limits_{w,b} L(w,b,\alpha)=\sum_{i=1}^{m}\alpha_i-\frac{1}{2}\sum_{i=1}^{m}\sum_{j=1}^{m}\alpha_i\alpha_jy^{(i)}y^{(j)}<x^{(i)},x^{(j)}> \nonumber\\
                &\textcircled{\footnotesize{3}} ~\text{再对上式求关于$~\alpha~$的极大}  \nonumber\\
                &\max \limits_\alpha ~~~\sum_{i=1}^{m}\alpha_i-\frac{1}{2}\sum_{i=1}^{m}\sum_{j=1}^{m}\alpha_i\alpha_jy^{(i)}y^{(j)}<x^{(i)},x^{(j)}> \nonumber\nonumber\\
...
\begin{figure}[!h]
  \centering
  \includegraphics[width=0.8\textwidth]{soft.eps}
  \caption{软间隔支持向量机}
\end{figure}
...
\indent\textbf{对偶问题} ~~
\begin{align}
   &L(w,b,\alpha,\xi,\mu) ~=~ \frac{1}{2}||w||^2+C\sum_{i=1}^{m}\xi_i-\sum_{i=1}^{m}\alpha_i~[y^{(i)}(w^Tx^{(i)}+b)-1+\xi_i]-\sum_{i=1}^{m}\mu_i\xi_i \nonumber\\
   &\theta_D(\alpha)~=~\min \limits_{w,b,\xi} L(w,b,\xi,\alpha,\mu)   \nonumber\\
   \text{求解：} &\textcircled{\footnotesize{1}} ~\text{先最小化lagrangian.$~L(w,b,\xi,\alpha,\mu)~$}  \nonumber\\
                &\nabla_wL(w,b,\xi,\alpha,\mu)=w-\sum_{i=1}^{m}\alpha_iy^{(i)}x^{(i)}=0 \nonumber\\
                &\textcolor{red}{w = \sum_{i=1}^{m}\alpha_iy^{(i)}x^{(i)}} \nonumber \\
                &\frac{\partial}{\partial b} L(w,b,\xi,\alpha,\mu) = \sum_{i=1}^{m}\alpha_iy^{(i)}=0 \nonumber\\
                &\textcolor{red}{\sum_{i=1}^{m}\alpha_iy^{(i)}=0} \nonumber \\
                &\nabla_{\xi}L(w,b,\xi,\alpha,\mu)=C-\alpha_i-\mu_i=0 \nonumber\\
                &\textcolor{red}{C-\alpha_i-\mu_i=0} \nonumber \\
                &\textcircled{\footnotesize{2}} ~\text{代入到原式：}\nonumber\\
                &\min \limits_{w,b,\xi} L(w,b,\xi,\alpha,\mu) \nonumber \\
                &=\sum_{i=1}^{m}\alpha_i-\frac{1}{2}\sum_{i=1}^{m}\sum_{j=1}^{m}\alpha_i\alpha_jy^{(i)}y^{(j)}<x^{(i)},x^{(j)}>\nonumber\\
                \theta_D(\alpha)&=~\min \limits_{w,b,\xi} L(w,b,\xi,\alpha,\mu)=\sum_{i=1}^{m}\alpha_i-\frac{1}{2}\sum_{i=1}^{m}\sum_{j=1}^{m}\alpha_i\alpha_jy^{(i)}y^{(j)}<x^{(i)},x^{(j)}> \nonumber\\
                &\textcircled{\footnotesize{3}} ~\text{再对上式求关于$~\alpha~$的极大}  \nonumber
\end{align}
\begin{align}
            &\max \limits_\alpha ~~~\sum_{i=1}^{m}\alpha_i-\frac{1}{2}\sum_{i=1}^{m}\sum_{j=1}^{m}\alpha_i\alpha_jy^{(i)}y^{(j)}<x^{(i)},x^{(j)}> \nonumber\nonumber\\
         s.&t.~~C-\alpha_i-\mu_i=0,~~~\alpha_i \geq 0,~~~\mu_i\geq 0, ~~~i=1,2,...,m \nonumber \\
               &~~~~~~~~~~~~~~~~~~~~~~~~~~~\sum_{i=1}^{m}\alpha_iy^{(i)}=0 \nonumber
\end{align}
\textbf{得到对偶问题} ~~
\begin{align}
...
\begin{align}
    ...
    &=e^{-\frac{x_1^2+x_2^2}{2\sigma^2}}e^{\frac{x_1x_2}{\sigma^2}}\nonumber\\
    &=e^{-\frac{x_1^2+x_2^2}{2\sigma^2}}\Big(1+\frac{1}{\sigma^2}\cdot\frac{x_1x_2}{1!}+(\frac{1}{\sigma^2})^2\cdot\frac{(x_1x_2)^2}{2!}+\cdots+(\frac{1}{\sigma^2})^n\cdot\frac{(x_1x_2)^n}{n!}+\cdots\Big)\nonumber\\
    &=e^{-\frac{x_1^2+x_2^2}{2\sigma^2}}\Big(1\cdot1+\frac{1}{1!}\frac{x_1}{\sigma}\frac{x_2}{\sigma}+\frac{1}{2!}\frac{x_1^2}{\sigma^2}\frac{x_2^2}{\sigma^2}+\cdots+\frac{1}{n!}\frac{x_1^n}{\sigma^n}\frac{x_2^n}{\sigma^n}+\cdots\Big)\nonumber\\
    &=\Phi(x_1)^T\Phi(x_2)\nonumber\\
    \Phi(x)&=e^{-\frac{x^2}{2\sigma^2}}\Big(1,\sqrt{\frac{1}{1!}}\frac{x}{\sigma},\sqrt{\frac{1}{2!}}\frac{x^2}{\sigma^2},\cdots,\sqrt{\frac{1}{n!}}\frac{x^n  }{\sigma^n},\cdots\Big) \nonumber
\end{align}
所以高斯核的$~\Phi(x)~$是无限维的。
...
\indent核函数一定是实对称矩阵。\\
\indent因此一定能相似对角化。\\
\indent$K = Q\Lambda Q^T$ 也就意味着  $\Lambda=Z^TKZ$
\begin{align}
\Lambda&=Z^TKZ\nonumber\\
...
&\geq 0 \nonumber
\end{align}
\indent因此$~\Lambda~$是半正定的，所以K也就是半正定的。
...
\begin{align}
    ...
    &~~~~~~~~~~~~~~~~=\min \limits_{\alpha_1,\alpha_2}\frac{1}{2}K_{11}\alpha_1^2+\frac{1}{2}K_{22}\alpha_2^2+y_1y_2K_{12}\alpha_1\alpha_2 \nonumber \\
    &~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-(\alpha_1+\alpha_2)+y_1v_1\alpha_1+y_2v_2\alpha_2+Constant   \tag{1}  \\
    &~~~~~~~~~~~~~~~~~~~~~~~~~~~ v_i = \sum_{j=3}^{m} \alpha_jy_jK(x_i,x_j) \nonumber \\
...
\begin{align}
    &~~~~~~~~~~~~~~~~\min \limits_{\alpha_2}\phi(\alpha2)=\frac{1}{2}K_{11}((\xi-\alpha_2y_2)y_1)^2+\frac{1}{2}K_{22}\alpha_2^2+y_1y_2K_{12}(\xi-\alpha_2y_2)y_1\alpha_2 \nonumber \\
    &~~~~~~~~~~~~~~~~~~~~~~~~-((\xi-\alpha_2y_2)y_1+\alpha_2)+y_1v_1(\xi-\alpha_2y_2)y_1+y_2v_2\alpha_2+Constant
\end{align}
\indent 利用微分求这个最小化问题的最优解：
 \begin{align}
    &\frac{\partial\phi(\alpha_2)}{\partial\alpha_2}=(K_{11}+K_{22}-2K_{12})\alpha_2-K_{11}\xi y_2 \nonumber \\
    &~~~~~~~~~~~~~~~~~+K_{12}\xi y_2+y_1y_2-1-v_1y_2+v_2y_1=0    \nonumber \\
    ...
    &~~~~~~\text{由于}~v_i = \sum_{j=3}^{m} \alpha_jy_jK(x_i,x_j)~\text{因此：} \nonumber \\
    &~~~~~~~~~~~~~~~~~~~~~~~v_1=f(x_1)-\sum_{j=1}^{2}\alpha_jy_jK_{1j}-b \\
    &~~~~~~~~~~~~~~~~~~~~~~~v_2=f(x_2)-\sum_{j=1}^{2}\alpha_jy_jK_{2j}-b \\
    &\text{将(4)(6)(7)代入求导式：}  \nonumber \\
    &~~~~~~(K_{11}+K_{22}-2K_{12})\alpha_2-(\alpha_1^{old}y_1+\alpha_2^{old}y_2)y_2(K_{11}-K_{12})+y_1y_2-y_2^2 \nonumber\\
...
\begin{align}
    ...
    &H=\min(0,C-K)=\min(C,C+\alpha_2^{old}-\alpha_1^{old}) \nonumber
\end{align}
\begin{figure}[!htb]
  \centering
...
\begin{align}
    ...
    &H=\min(K,C)=\min(C,\alpha_2^{old}+\alpha_1^{old}) \nonumber
\end{align}
\indent 裁剪后：
...
\indent在S个数据集建好之后，将某个学习算法分别作用于每个数据集就得到了S个分类器。当我们要对新数据进行分类时，就可以应用这S个分类器进行分类。与此同时，选择分类器投票结果中最多的类别作为最后的分类结果。\\
\indent当然，还有一些更先进的bagging方法，比如随机森林（random forest）。
...
\indent因boosting分类的结果是基于所有分类器的加权求和结果的，boosting与bagging不太一样。bagging中的分类器权重是相等的，而boosting中的分类器权重并不相等，每个权重代表的是其对应分类器在上一轮迭代中的成功度。
...
\begin{figure}[!h]
  \centering
  \includegraphics[width=0.78\textwidth]{Adaboost.eps}
  \caption{算法流程}
\end{figure}
...
\begin{figure}[!h]
  \centering
  \includegraphics[width=1\textwidth]{TFPN.eps}
\end{figure}
...
\begin{figure}[!h]
  \centering
  \includegraphics[width=1\textwidth]{costMatrix.eps}
  \caption{代价矩阵}
\end{figure}
